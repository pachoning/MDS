...
fast_mds_3 <- function(
  ...
  for(i_group in 1:p){
    ...
    if( n_sample > nrow( row.names(x)[ind]) ) n_sample = nrow( row.names(x)[ind]) 
    
    list_index[[i_group]] = sample(
      x = 1:nrow( list_matrix[[i_group]] ), 
      size = n_sample, 
      replace = FALSE
    )
    ...
    ind_M = list_index[[i_group]]
    sub_matrix =  list_matrix[[i_group]][ind_M, ]
  ...
  if( total_groups > 0){
    ...
    for( l_group in 1:total_groups ){
      ...
      ind_M = which( row.names( x_M_align ) %in% rows_names_i )
