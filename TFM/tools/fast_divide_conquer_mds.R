  ...
  for(k in 1:total_groups){
    ...
    if(k == 1){
      ...
      filter_rows_by_position = c(
        x_position_subsample_previous_group,
        x_positions_current_group
      )
    ...
    if(k == 1){
      ...
      positions_previous_group_current_mds = row.names(mds_iteration) %in% row.names(x)[x_position_subsample_previous_group]  
      ...
      positions_cum_sum_previous = which(row.names(cum_mds) %in% row.names(x)[x_position_subsample_previous_group])
