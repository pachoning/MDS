...
for (j in 1:J) {
  results_folder_name <- paste0("all_projections_", j)
  experiment_label = "all_projections"
...
for (j in 1:J) {
  ...
  experiment_label = "rmds_interpolation"

  sample_size = c(5*10^5, 10^6)
  n_cols = c(10)

  distribution_parameters <- list(list(var = rep(15, 5)))

  scenarios <- list(sample_size = sample_size, n_cols = n_cols, distribution_parameters = distribution_parameters)
  algorithms <- list(gower = gower_interpolation_mds, emmanuel = emmanuel_mds)
