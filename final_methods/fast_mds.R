...
fast_mds <- function(x, l, s, k, dist_fn = stats::dist, ...) {
  ...
  if (n <= l) {
    ...
    mds_partition <- parallel::mclapply(x_partition, 
                                        fast_mds, 
                                        l = l, 
                                        s = s, 
                                        k = k, 
    ...
    x_partition_sample <- parallel::mcmapply(function(matrix, idx) { matrix[idx, , drop = FALSE] },
    ...
    ini_index <- parallel::mcmapply(function(i, partition) (i-1)*length(partition) + 1,
                                    i = 1:num_partition, partition = sample_partition,
                                    SIMPLIFY = FALSE)
    end_index <- parallel::mcmapply(function(i, partition) i*length(partition),
                                    i = 1:num_partition, partition = sample_partition,
                                    SIMPLIFY = FALSE)
    ...
    mds_M_sampling_points <- parallel::mcmapply(function(matrix, ini, end) {  matrix[ini:end, , drop = FALSE] },
    ...
    mds_partition_sampling_points <- parallel::mcmapply(function(matrix, idx) { matrix[idx, , drop = FALSE] }, 
                                                        matrix = mds_partition_points, idx = sample_partition, SIMPLIFY = FALSE)
    
    # Apply Procrustes
    procrustes <- parallel::mcmapply(perform_procrustes, x = mds_partition_sampling_points, target = mds_M_sampling_points, 
    ...
    # Build GOF metric
    GOF <- parallel::mcmapply(function(x, y) x*nrow(y), x = mds_GOF, y = x_partition, SIMPLIFY = FALSE)
    GOF <- Reduce(`+`, GOF)/n
