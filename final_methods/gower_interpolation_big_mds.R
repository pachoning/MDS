...
get_partitions_for_gower_interpolation_big <- function(n, l) {
  ...
  if (p>1) {
    ...
    last_part <- permutation[(l + 1):n]
    list_index <- list(first_part, last_part)
  } else {
...
gower_interpolation_formula_big <- function(A, constant, q_product_all, q_product_last, X_1__S_inv) {
...
gower_interpolation_big_mds <- function(x, l, k, dist_fn = stats::dist, ...) {
  
  n <- nrow(x)
  idexes_partition <- get_partitions_for_gower_interpolation_big(n = n, l = l)
  ...
  if (num_partitions <= 1) {
    ...
    MDS <- parallel::mclapply(A, 
                              gower_interpolation_formula_big, 
                              constant = constant,
